(function () {
  ...
  function findTransformerByKva(kva, phaseHint) {
  ...
  function buildTrInstallLines(intent, master) {
    ...
    const transformer = intent.transformerId
      ...
      : findTransformerByKva(kva, phase);
  ...
  function parseQueryLocal(query, budgetType = "01.1") {
    ...
    if (isTransformer && kva) {
      ...
      const transformer = findTransformerByKva(kva, phase);
      return {
  ...
  function buildQuote(intent, master) {
    ...
    const breakdown = calculateQuoteTotal(lines, budgetType);
